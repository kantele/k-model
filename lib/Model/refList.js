...
function patchToEvent(type, segments, eventArgs, refList) {
  ...
  if (type === 'change' || type === 'load' || type === 'unload') {
    ...
    if (type === 'change') {
      ...
      // If this is a path query, we need to find out where to insert this document
      value = eventArgs[0];
      previous = undefined;
    } else if (type === 'unload') {
      value = undefined;
    ...
    if (newIndices) {
      for (var i = 0; i < newIndices.length; i++) {
        ...
        if (refList.options.isPathQuery) {
          var current = model._get(refList.fromSegments);
          // is there a correct doc in the outSegments?
          var existsValue = model._get(outSegments);
          if (current && current.length > newIndices[i] && (typeof existsValue === 'undefined' || (existsValue && value && existsValue.id == value.id))) {
            model._set(outSegments, value);
          }
          // If not, do 'insert' instead of 'set'
          // TODO: rethink this whole scenario in light of the new path query implementation.
          // Maybe we need to go through the ids and do a 'push' instead of insert to make sure
          // there won't be any gaps in the array (undefineds).
          else {
            model._insert(refList.fromSegments, newIndices[i], [value]);
          }        
        }
        else {
          var outSegments = refList.fromSegments.concat(newIndices[i]);
...
function patchIdsEvent(type, segments, eventArgs, refList) {
  ...
  if (segmentsLength === idsLength) {
    if (type === 'insert') {
      ...
      // pathquery is handled a bit differently
      if (refList.options.isPathQuery) {
        for (var i = 0; i < inserted.length; i++) {
          var value = refList.options.idlist? inserted[i]: refList.itemById(inserted[i]);

          // if this is a pathquery, the value may not have arrived yet
          // if so, we return now and inserting will be tried in the 'load' event
          // TODO: make sure "index++" way of doing this works.
          model._insert(refList.fromSegments, index++, [value]);
        }
      }
      else {
        for (var i = 0; i < inserted.length; i++) {
...
RefList.prototype.get = function() {
  ...
  if (this.options.isPathQuery) {
    var ids = this.model._get(this.ids.split('.'));
    return this.model.getDocs(this.to, ids, this.options);
  }
...
RefList.prototype.indicesByItem = function(item) {
  ...
  var ids = this.model._get(this.idsSegments);
  // is this needed to be done like that: 
  // var ids = this.options.isPathQuery? this.model._get(this.ids.split('.')): this.model._get(this.idsSegments);
  if (!ids) return;
  var indices;
  var index = -1;
  for (;;) {
