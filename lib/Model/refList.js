...
function patchToEvent(type, segments, eventArgs, refList) {
  ...
  if (segmentsLength > toLength + 1) {
    ...
    if (refList.options && refList.options.byid) {
      var id = refList.idByItem(value);
      if (!id) return;
      ...
      console.log('dereferenced (2)', dereferenced);
  ...
  if (type === 'change' || type === 'load' || type === 'unload') {
    ...
    if (refList.options && refList.options.byid) {
      ...
      if (oldIndices && oldIndices !== newIndices) {
...
function patchIdsEvent(type, segments, eventArgs, refList) {
  ...
  // the output array
  if (segmentsLength === idsLength && (!refList.options || !refList.options.byid)) {
    if (type === 'insert') {
      var index = eventArgs[0];
...
RefList.prototype.get = function() {
  ...
  if (this.options && this.options.byid) {
    return items;
  }
