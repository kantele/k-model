...
Model.prototype.subscribeDoc = function(collectionName, id, cb, persistent) {
  // console.log('** Model.prototype.subscribeDoc ('+collectionName+')', this.root && this.root.collections &&  this.root.collections.auths && this.root.collections.auths.docs);
  var path = collectionName + '.' + id,
      model = this,
...
Model.prototype.bulkUnsubscribeDocs = function(docs, cb) {
  ...
  function getCb(doc, path) {
    return function unsubscribeDocCallback() {
      model._maybeUnloadDoc(doc.c, doc.id, path);
    }
  }
}
...
Model.prototype._maybeUnloadDoc = function(collectionName, id, path) {
  ...
  // console.log('_maybeUnloadDoc', path, count);
  if (count) {
    return;
  }
