  ...
  for (var i = 0; i < args.length; i++) {
    ...
    if (item instanceof Query) {
      ...
      if (segments.length === 2) {
        ...
        this[docMethod](segments[0], segments[1], group(), 'item', item && item._persistent);
...
Model.prototype.subscribeDoc = function(collectionName, id, cb, persistent) {
  var path = collectionName + '.' + id,
      count = this.root._subscribedDocs[path] = (this.root._subscribedDocs[path] || 0),
      model = this,
  ...
  if (!doc._persistent || !count) {
    this.root._subscribedDocs[path]++;
...
Model.prototype.unsubscribeDoc = function(collectionName, id, cb, followPersistency) {
  var model = this,
      ...
      subscribedDocs = this.root._subscribedDocs;

  cb = this.wrapCallback(cb);

  if (followPersistency && doc && doc._persistent) {
    return cb();
  }
