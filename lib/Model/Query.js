...
Query.prototype.subscribeOLD = function(cb, rpcClientCall) {
  var self = this;
  cb = this.model.wrapCallback(cb);

  if (this.persistent && this.subscribeCount) {
    // if this is a path query, a persistent query and the query has changed, let's
    // send the new query to the server.
    if (this.isPathQuery) {
      var newQuery = this.dbQuery();

      if (!arrayEquals(newQuery, this.shareQuery.query)) {
...
Query.prototype.unfetch = function(cb, followPersistency) {
  cb = this.model.wrapCallback(cb);
...
Query.prototype.unsubscribe = function(cb, followPersistency) {
  var query = this;
