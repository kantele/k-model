...
Model.prototype._initQueries = function(items) {
  ...
  for (var i = 0; i < items.length; i++) {
    ...
    var source = item[6];
    var extra = item[7];
    var query = this.query(collectionName, expression, source);
...
Query.prototype._setResultIds = function(ids) {
  ...
  var previousIds = this.getIds();
  // Reset the map of ids, which is checked in maybeUnload
  this.idMap = {};
  this._addMapIds(ids);
  this.model._setArrayDiff(this.idsSegments, ids);
  this._maybeUnloadDocs(previousIds);
};
Query.prototype._registerDocs = function(ids) {
  ...
  for (var i = 0; i < ids.length; i++) {
    ...
    if (!doc) {
      doc = this.model.getOrCreateDoc(this.collectionName, id);
      var segments = [this.collectionName, id];
