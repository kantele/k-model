...
Model.INITS.push(function(model) {
  ...
  model.on('all', function(segments) {
    ...
    for (var hash in map) {
      for (var i = 0; i < map[hash].length; i++) {
        var query = map[hash][i];
...
Queries.prototype.add = function(query) {
  if (!this.map[query.hash]) {
    this.map[query.hash] = [];
  }

  // prevent duplicates
  for (var i = 0; i < this.map[query.hash].length; i++) {
    if (this.map[query.hash][i] === query) {
      return;
    }
  }

  this.map[query.hash].push(query);
};
Queries.prototype.remove = function(query) {
  for (var i = 0; i < this.map[query.hash].length; i++) {
    if (this.map[query.hash][i] === query) {
      this.map[query.hash].splice(i, 1);
    }
  }
};
Queries.prototype.get = function(collectionName, expression, source) {
  var hash = expression? queryHash(collectionName, expression, source): collectionName;
  return this.map[hash] && this.map[hash][0];
};
Queries.prototype.toJSON = function() {
  var out = [];
  for (var hash in this.map) {
    for (var i = 0; i < this.map[hash].length; i++) {
      var query = this.map[hash][i];
...
Query.prototype.create = function() {
  // console.log('Query.prototype.create', this);
  this.created = true;
  this.model.root._queries.add(this);
};

Query.prototype.destroy = function() {
  // console.log('Query.prototype.destroy', this);
...
Query.prototype.subscribe = function(cb) {
  // console.log('Query.prototype.subscribe', this);
  cb = this.model.wrapCallback(cb);
...
Query.prototype.unsubscribe = function(cb) {
  // console.log('Query.prototype.unsubscribe', this);
  cb = this.model.wrapCallback(cb);
  ...
  function finishUnsubscribeQuery() {
    ...
    var ids;
    if (query.shareQuery) {
      ids2 = ids = resultsIds(query.shareQuery.results);
    ...
    if (!query.model.root.fetchOnly && ids && ids.length) {
      if (ids.length > 1) {
        ...
        for (var i = 0; i < ids.length; i++) {
          docs.push({ c: query.collectionName, id: ids[i] });
        }

        query.model.bulkUnsubscribeDocs(docs, group());
      }
      else {
        // Unsubscribe all documents that this query currently has in results
        var group = util.asyncGroup(unsubscribeQueryCallback);
