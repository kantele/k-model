...
var defaultType = require('k-sync').types.defaultType;
...
Query.prototype.setQuery = function(expression) {
  ...
  // Also, only se this here if this query is not a path query
  // because ...
  if (!this.isPathQuery) {
    this.expression = expression;
  }
...
Query.prototype.subscribe = function(cb) {
  ...
  if (!this.model.root.rpcServerCall) {
    ...
    if (this.model.root.rpcClientCall) {
      setTimeout(send, 0);
    }
    else {
      send();
...
Query.prototype.serialize = function() {
  ...
  if (collection) {
    ...
    for (var i = 0; i < ids.length; i++) {
      var id = ids[i];
      var doc = collection.docs[id];
      // don't send collection data with queries unless its graph data
      // the collection data is sent with "collections"
      // queries can be recreated from that
      if (this.isGraph && doc) {
