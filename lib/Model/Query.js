...
Model.prototype.query = function(collectionName, expression, options) {
  ...
  if (isPathQuery) {
    query = new PathQuery(this, collectionName, expression, options);
  }
  else {
...
// For rpc calls -- called after rpc, because id lists may have changed
Model.prototype._refreshPathQueries = function() {
  var queries = this.root._queries;

  for (var i in queries.map) {
    if (queries.map[i].isPathQuery) {
      if (queries.map[i].subscribeCount) {
        queries.map[i].subDocs(null, queries.map[i].getIds());
      }
      else if (queries.map[i].fetchCount) {
        queries.map[i]._fetch(function() {});
      }
    }
  }
}
  ...
  if (items) {
    for (var i = 0; i < items.length; i++) {
      ...
      if (expression && expression.$g) {
        var query = this.graph(collectionName, expression, options);
      }
      else if (expression && expression.$n) {
        var query = this.notification(collectionName, expression, options.persistent);
      }
      else {
...
Queries.prototype.reConstruct = function(model) {
  for (var q in this.map) {
    if (this.map[q].setListeners) this.map[q].setListeners()
  }
}
...
function PathQuery(model, collectionName, expression, options) {
  Query.call(this, model, collectionName, expression, options);
}

// inherit Query
PathQuery.prototype = Object.create(Query.prototype);

// correct the constructor pointer because it points to Query
PathQuery.prototype.constructor = PathQuery;

PathQuery.prototype._fetch = function(cb) {
  var ids = this.getIds();
  var docs = [];

  for (var i = 0; i < ids.length; i++) {
    docs.push(this.model.at(this.collectionName + '.' + ids[i], this.options.persistent))
  }

  this.model.fetch(docs, cb);

  return this;
}

PathQuery.prototype.fetch = function(cb) {
  this.model._context.fetchQuery(this);
  this.fetchCount++;

  return this._fetch(cb);
}

PathQuery.prototype.subscribe = function(cb, rpcClientCall) {
  var self = this;

  self.model._context.subscribeQuery(self);
  self.subscribeCount++;

  function sub() {
    var ids = self.getIds();
    self.subDocs(null, ids, cb);
    self.setListeners();
  }
...
PathQuery.prototype.subDocs = function(idx, ids, cb) {
  var docs = [];
  ...
  if (ids)
    for (var i = 0; i < ids.length; i++) {
      // console.log('subscribe to', this.collectionName + '.' + ids[i], this.options.persistent);
      docs.push(this.model.at(this.collectionName + '.' + ids[i], this.options.persistent))
    }

  this.model.subscribe(docs, cb);
}

PathQuery.prototype.setListeners = function() {
  var self = this;

  if (this.listenersSet) return;

  this.model.on('insert', this.expression.join('.'), this.subDocs.bind(this));
  this.model.on('change', this.expression.join('.'), function(ids) { self.subDocs(null, ids); });
  this.listenersSet = true;
}

PathQuery.prototype.getIds = function() {
  return this.model._get(this.expression) || [];
};

PathQuery.prototype.get = function() {
  return this.model.getDocs(this.collectionName, this.getIds(), this.options);
};

PathQuery.prototype.ref = function(from) {
  return this.model.refList(from, this.collectionName, this.expression.join('.'), { byid: this.options.byid, isPathQuery: 1 });
};
...
//   if (this.isPathQuery) {
//     return pathIds(this.model, this.expression);
//   }
...
Query.prototype.fetch = function(cb) {
  ...
  var query = this,
      model = this.model,
      options = (this.options) ? util.copy(this.options) : {};
...
Query.prototype.subscribe = function(cb, rpcClientCall) {
  ...
  if (this.persistent && this.subscribeCount) {
    cb();
    return this;
  }
...
Query.prototype.unsubscribePersistent = function(cb) {
  this.unsubscribe(cb, true);
}

Query.prototype.unsubscribe = function(cb, unsubscribePersistent) {
  var query = this;
  ...
  if (this.persistent && !unsubscribePersistent) {
    return cb();
  }

  // console.log('Query.prototype.unsubscribe', this.expression)
  ...
  if (!this.subscribeCount) {
    ...
    if (!this.persistent || unsubscribePersistent) destroyShareQuery();
...
Query.prototype.get = function() {
  ...
  if (!data) {
    console.warn('You must fetch or subscribe to a query before getting its results. (' + this.segments.join('.') + ')');
    return results;
  }
  var ids = data.ids;
...
Query.prototype.ref = function(from) {
  return this.model.refList(from, this.collectionName, this.idsSegments.join('.'), { byid: this.options.byid });
};
