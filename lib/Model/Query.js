  ...
  model.on('all', function(segments) {
    ...
    for (var hash in map) {
      ...
      if (query.isPathQuery && query.shareQuery && util.mayImpact(query.expression, segments)) {
        ...
        // we will send the query if:
        // - shareQuery isn't 'ready', meaning it's usually in transit
        // - the previousIds and ids differ
        if ((query && query.shareQuery && !query.shareQuery.ready) || !arrayEquals(previousIds, ids)) { 
  ...
  if (this.persistent && this.subscribeCount) {
    ...
    if (this.isPathQuery) {
      ...
      if (!arrayEquals(newQuery, this.shareQuery.query)) {
        // console.log('updating query...', this.hash);
        // console.log('updating query...', this.hash, this.shareQuery.query, newQuery);
