...
Model.prototype.query = function(collectionName, expression, options) {
  options = options || {};
  var query = this.root._queries.get(collectionName, expression, options.db);
...
Model.prototype._initQueries = function(items) {
  ...
  for (var i = 0; i < items.length; i++) {
    ...
    if (expression && expression.$graph) {
      var query = this.graph(collectionName, expression, db);
    }
    else {
      var query = this.query(collectionName, expression, db);
    }
...
Queries.prototype.get = function(collectionName, expression, db) {
  var hash = expression? queryHash(collectionName, expression, db): collectionName;
...
Query.prototype.fetch = function(cb) {
  ...
  if (this.deleted) {
    console.warn('Trying to fetch with a deleted query', query.hash);
    return;
  }
  ...
  this.model.root.shareConnection.createFetchQuery(
    this.collectionName,
    this.expression,
    options,
    fetchCb
...
Query.prototype.get = function() {
  ...
  for (var i = 0, l = ids.length; i < l; i++) {
    ...
    this.options.byid? results[id] = doc && doc.get(): results.push(doc && doc.get());
  }
  return results;
};
