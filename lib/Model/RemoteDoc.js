...
function RemoteDoc(model, collectionName, id, snapshot, collection) {
  ...
  this.shareDoc = model.root.connection.get(collectionName, id, this.isGraph);
...
RemoteDoc.prototype._initShareDoc = function() {
  ...
  shareDoc.on('op', function(op, isLocal) {
    ...
    if (shareDoc && shareDoc.type && shareDoc.type.name !== 'rich-text') {
      doc._onOp(op);
    }
  });
...
RemoteDoc.prototype.set = function(segments, value, cb) {
  ...
  var op = isArray ?
    ...
    [new ObjectReplaceOp(segments, previous, value, isArray)];
...
RemoteDoc.prototype._createImplied = function(segments) {
  if (!this.shareDoc.type) {
    console.error('Mutation on uncreated remote document ('+this.collectionName + '.' + this.id + ' : ' + segments+')');
  }
