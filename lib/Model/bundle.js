...
Model.prototype.bundle = function(cb) {
  ...
  root.whenNothingPending(function finishBundle() {
    ...
    stripComputed(root);
    bundle.collections = root.serializeCollections(root);
...
Model.prototype.serializeCollections = function(root) {
  var out = {};
  for (var collectionName in root.collections) {
    ...
    for (var id in collection.docs) {
      ...
      // it will be recreated from reflists
      if (this._refLists.fromMap[collectionName+'.'+id]) {
        continue;
      }
