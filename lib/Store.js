...
function Store(racer, options) {
  ...
  this.backend = (options && options.backend) || new Sync(options);
  this.shareClient = this.backend; // DEPRECATED
  ...
  this.on('bundle', function(browserify) {
    browserify.require(__dirname + '/index.js', {expose: 'k-model'});
  });
}
